...
class MetadataSynthesizer(object):
    def __init__(
        ...
        self._nb_mixtures = self._mixture_setup['nb_folds'] * self._nb_mixtures_per_fold if np.isscalar(
    ...
    def create_mixtures(self):
        ...
        for nfold in range(self._mixture_setup['nb_folds']):
            ...
            for na in range(self._nb_active_classes):
                idx_active = np.append(idx_active, np.nonzero(
                    self._db_config._samplelist[nfold]['class'] == self._active_classes[na]))
            ...
            for nr in range(nb_rooms_nf):
                ...
                if np.isscalar(self._nb_mixtures_per_fold):
                    ...
                    nb_mixtures_per_fold_per_room = int(
                        np.round(self._nb_mixtures_per_fold[nfold] / float(nb_rooms_nf)))
                ...
                for nmix in range(nb_mixtures_per_fold_per_room):
                    ...
                    nth_mixture = {'files': np.array([]), 'class': np.array([]), 'event_onoffsets': np.array([]),
                                   'sample_onoffsets': np.array([]), 'trajectory': np.array([]),
                                   'isMoving': np.array([]), 'isFlippedMoving': np.array([]),
                    ...
                    for layer in range(self._mixture_setup['nOverlap']):
                        ...
                        if len(foldlist_nf['onoffset'][event_idx_in_layer[-1]]) == 1:
                            ons = foldlist_nf['onoffset'][event_idx_in_layer[-1]][0][0, 0] if self._db == 'nigens' else \
                            foldlist_nf['onoffset'][event_idx_in_layer[-1]][0][0]
                        else:
                            ons = foldlist_nf['onoffset'][event_idx_in_layer[-1]][0, 0] if self._db == 'nigens' else \
                            foldlist_nf['onoffset'][event_idx_in_layer[-1]][0]
                        if (trimmed_event_length > self._trim_threshold) and (
                                trimmed_event_length > np.floor(ons * 10.) / 10.):
                        ...
                        for nl in range(nb_samples_in_layer):
                            ...
                            if is_moving:
                                ...
                                if is_flipped_moving:
                                    ...
                                    if ev_nspeed + 1:
                                        end_idx = event_span_nl + self._rnd_generator.integers(0,
                                                                                               nRirs - event_span_nl + 1)
                                        start_idx = end_idx - event_span_nl
                                        riridx = start_idx + np.arange(0, event_span_nl, dtype=int)
                                        riridx = riridx[np.arange(0, len(riridx), ev_speed / 10,
                                                                  dtype=int)]  # pick every nth RIR based on speed
                            ...
                            if nl == 0 and layer == 0:
                                nth_mixture['event_onoffsets'] = np.array(
                                    [[time_idx / 10., (time_idx + event_duration_nl) / 10.]])
                                nth_mixture['doa_azel'] = [cart2sph(traj_doas[ev_traj][riridx, :])]
                                nth_mixture['sample_onoffsets'] = [sample_onoffsets]
                            else:
                                nth_mixture['event_onoffsets'] = np.vstack((nth_mixture['event_onoffsets'], np.array(
                                    [time_idx / 10., (time_idx + event_duration_nl) / 10.])))
                            ...
                            nth_mixture['class'] = np.append(nth_mixture['class'], self._class2activeClassmap[
                                int(foldlist_nf['class'][event_nl])])
                            ...
                            nth_mixture['isFlippedMoving'] = np.append(nth_mixture['isFlippedMoving'],
                                                                       is_flipped_moving)
    ...
    def prepare_metadata_and_stats(self):
        ...
        self._metadata = []
        stats = {}
        stats['nFrames_total'] = self._mixture_setup[
                                     'nb_folds'] * self._nb_mixtures_per_fold * self._nb_frames if np.isscalar(
            self._nb_mixtures_per_fold) else np.sum(self._nb_mixtures_per_fold) * self._nb_frames
        ...
        for nfold in range(self._mixture_setup['nb_folds']):
            ...
            for nr in range(nb_rooms):
                ...
                for nmix in range(nb_mixtures):
                    mixture = {'classid': np.array([]), 'trackid': np.array([]), 'eventtimetracks': np.array([]),
                    ...
                    for nev in range(nb_events):
                        ...
                        if len(sample_shape) == 1:
                            activity_mask[np.arange(int(np.round(sample_onoffsets[0] * 10)),
                                                    int(np.round(sample_onoffsets[1] * 10)))] = 1
                        else:
                            for nseg in range(sample_shape[0]):
                                ran = np.arange(int(np.round(sample_onoffsets[nseg, 0] * 10)),
                                                int(np.round((sample_onoffsets[nseg, 1]) * 10)))
                                activity_mask[ran] = 1
                        ...
                        if np.shape(doa_azel)[0] == 1:
                            ...
                            try:
                                ...
                                eventdoatimetracks[ev_idx[activity_mask.astype(bool)], 0, nev] = np.ones(
                                    np.sum(activity_mask == 1)) * doa_azel[0, 0]
                                eventdoatimetracks[ev_idx[activity_mask.astype(bool)], 1, nev] = np.ones(
                                    np.sum(activity_mask == 1)) * doa_azel[0, 1]
                            except IndexError:
                            ...
                            try:
                                eventtimetracks[ev_idx, nev] = activity_mask
                                eventdoatimetracks[ev_idx[activity_mask.astype(bool)], :, nev] = doa_azel[
                                                                                                 activity_mask.astype(
                                                                                                     bool), :]
                            except IndexError:
                                excess_idx = len(np.argwhere(ev_idx >= self._nb_frames))
                                ev_idx = ev_idx[:-excess_idx]
                    ...
                    for nf in range(self._nb_frames):
                        ...
                        if nb_active > 0:
                            ...
                            for na in range(nb_active):
                                classidx = int(
                                    active_classes[na][0])  # additional zero index since it's packed in an array

...
class AudioMixer(object):
    def __init__(
        ...
        self._rooms_paths2020 = os.listdir(self._recpath2020)
        ...
        for nfold in range(self._nb_folds):
            ...
            for nr in range(nb_rooms):
                ...
                if self._scenarios[2]:
                    ...
                    temp_idx = int(
                        np.argwhere([int(roompath[i].split('_')[0]) for i in range(len(roompath))] == roomidx))
                    ambience, _ = soundfile.read(recpath  + '/' + roompath[temp_idx] + '/ambience_' + self._mic_format + '_24k_edited.wav')

...
class AudioMixer(object):
    def __init__(
            self, params, db_config, mixtures, mixture_setup, audio_format, scenario_out,
            scenario_interf='target_interf'
    ):
        ...
        for nfold in range(self._nb_folds):
            ...
            for nr in range(nb_rooms):
                ...
                if self._scenarios[2]:
                    ...
                    ambience, _ = soundfile.read(
                        recpath + '/' + roompath[roomidx - 1] + '/ambience_' + self._mic_format + '_24k_edited.wav')
                ...
                for nmix in range(nb_mixtures):
                    ...
                    target_omni_energy = np.sum(
                        np.mean(target_sig, axis=1) ** 2) if self._audio_format == 'mic' else np.sum(
                        target_sig[:, 0] ** 2)

                    if self._scenarios[1]:
                        print('Loading interferer mixture {}/{} \n'.format(nmix + 1, nb_mixtures))
                        interf_sig, _ = soundfile.read(
                            self._interfpath + '/' + self._audio_format + '/' + mixture_filename)
                        inter_omni_energy = np.sum(
                            np.mean(interf_sig, axis=1) ** 2) if self._audio_format == 'mic' else np.sum(
                            interf_sig[:, 0] ** 2)
                    ...
                    if self._scenarios[2]:
                        ...
                        ambi_energy = np.sum(
                            np.mean(ambient_sig, axis=1) ** 2) if self._audio_format == 'mic' else np.sum(
                            ambient_sig[:, 0] ** 2)
                    ...
                    soundfile.write(self._scenariopath + '/' + self._audio_format + '/' + mixture_filename, target_sig,
                                    self._fs_mix)
